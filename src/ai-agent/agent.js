...
${resolvedMessage}`;
  ...
  _buildSystemInstruction(
    ...
    const operatingPrinciples = [
      ...
      "- Good CMD uses include: `Get-Content`, `Get-ChildItem`, `Select-String`, `git status`, `git diff`, `npm run <script>`, `npm test`, `node --check`, and other focused workspace commands.",
      ...
      "- On Windows, NEVER use `rg`, `grep`, `find`, `cat`, `ls`, or `sed` in CMD:. Use the native `GREP:` action for searching, or PowerShell equivalents like `Select-String`.",
      ...
      "- If the user wants a live preview of a specific file or the active workspace, use `PREVIEW: open` (or `PREVIEW: open <path>` to force a specific file).",
      "- If the user wants you to inspect/study/analyze the live preview, detect render/runtime issues, or fix problems found from the preview, use `PREVIEW: inspect` (or `PREVIEW: inspect <path>`).",
