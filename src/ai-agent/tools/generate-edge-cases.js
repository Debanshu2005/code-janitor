...
const DIRECT_JAVASCRIPT_TEST_EXTENSIONS = new Set([".js"]);
const COMPANION_JEST_LANGUAGES = new Set(["typescript", "html", "c", "cpp"]);
...
function buildGeneratedTestMetadata(filePath, language) {
  const normalizedFilePath = normalizeWorkspacePath(filePath);
  const sourceExtension = path.extname(normalizedFilePath).toLowerCase();
  const sourceBaseName = path.basename(normalizedFilePath, sourceExtension);
  const sourceDirectory = path.posix.dirname(normalizedFilePath);

  if (language === "python") {
    return {
      testFramework: "pytest",
      testCodeLanguage: "python",
      testExecutionLanguage: "python",
      testFilePath: path.posix.join(
        sourceDirectory,
        "__tests__",
        `${sourceBaseName}.edge-cases.test.py`
      )
    };
  }
  ...
  if (language === "java") {
    return {
      testFramework: "junit",
      testCodeLanguage: "java",
      testExecutionLanguage: "java",
      testFilePath: path.posix.join(
        sourceDirectory,
        "__tests__",
        `${sourceBaseName}.edge-cases.test.java`
      )
    };
  }
  ...
  if (
    language === "javascript" &&
    DIRECT_JAVASCRIPT_TEST_EXTENSIONS.has(sourceExtension)
  ) {
    return {
      testFramework: "jest",
      testCodeLanguage: "javascript",
      testExecutionLanguage: "javascript",
      testFilePath: path.posix.join(
        sourceDirectory,
        "__tests__",
        `${sourceBaseName}.edge-cases.test.js`
      )
    };
  }

  if (
    COMPANION_JEST_LANGUAGES.has(language) ||
    (language === "javascript" && !DIRECT_JAVASCRIPT_TEST_EXTENSIONS.has(sourceExtension))
  ) {
    return {
      testFramework: "jest",
      testCodeLanguage: "javascript",
  ...
  return {
    ...
    testExecutionLanguage: language,
    testFilePath: path.posix.join(
      sourceDirectory,
      "__tests__",
      `${sourceBaseName}.edge-cases.test${sourceExtension}`
    )
  };
}

function buildCompanionJestTest(filePath, language, edgeCases, definitions = []) {
  const normalizedFilePath = normalizeWorkspacePath(filePath);
  const sourceBaseName = path.basename(
    normalizedFilePath,
    path.extname(normalizedFilePath)
  );
  const sourceRelativePath = buildJavaScriptRequirePath(normalizedFilePath) +
    path.extname(normalizedFilePath);
  const definitionNames = definitions
    .map((definition) => String(definition?.name || "").trim())
    .filter(Boolean)
    .slice(0, 5);
  const todoDescriptions = edgeCases
    .map((edgeCase) => String(edgeCase?.testCase?.description || "").trim())
    .filter(Boolean)
    .slice(0, 12);

  const expectationLines = definitionNames.map((name) => {
    const needle = name.includes(".") ? name.split(".").pop() : name;
    return `    expect(content).toContain(${JSON.stringify(needle)});`;
  });

  const todoLines = todoDescriptions.map(
    (description) => `  test.todo(${JSON.stringify(description)});`
  );

  const htmlSmokeAssertion =
    language === "html"
      ? `\n  test('contains an HTML root marker', () => {\n    const content = readSource();\n    expect(/<html|<!doctype html/i.test(content)).toBe(true);\n  });`
      : "";

  return `/**
 * Edge case smoke tests for ${normalizedFilePath}
 * Generated by Code Janitor
 *
 * These tests are intentionally lightweight and portable so they can run
 * in Jest even when the source language is not directly executable here.
 */

const fs = require('fs');
const path = require('path');

const SOURCE_FILE = path.join(__dirname, ${JSON.stringify(sourceRelativePath)});

function readSource() {
  return fs.readFileSync(SOURCE_FILE, 'utf8');
}

describe('${sourceBaseName} - Edge Cases (${language})', () => {
  test('source file exists and is not empty', () => {
    expect(fs.existsSync(SOURCE_FILE)).toBe(true);
    expect(readSource().trim().length).toBeGreaterThan(0);
  });

  test('expected definitions are still present', () => {
    const content = readSource();
${expectationLines.length > 0 ? expectationLines.join("\n") : "    expect(typeof content).toBe('string');"}
  });${htmlSmokeAssertion}

${todoLines.length > 0 ? todoLines.join("\n") : "  test.todo('Add source-specific edge cases for this file.');"}
});
`;
}
...
async function generateEdgeCases(filePath, workspaceRoot, executionContext = {}) {
  try {
    ...
    if (definitions.length === 0 && language !== "html") {
      return {
        success: false,
    ...
    const testMetadata = buildGeneratedTestMetadata(filePath, language);
    const testCode =
      testMetadata.testCodeLanguage === "javascript" &&
      testMetadata.testExecutionLanguage === "javascript" &&
      !(language === "javascript" && DIRECT_JAVASCRIPT_TEST_EXTENSIONS.has(path.extname(filePath).toLowerCase()))
        ? buildCompanionJestTest(filePath, language, mergedEdgeCases, definitions)
        : generateTestCode(mergedEdgeCases, language, filePath);
    
    return {
      success: true,
      filePath,
      language,
      testCodeLanguage: testMetadata.testCodeLanguage,
      testExecutionLanguage: testMetadata.testExecutionLanguage,
      testFramework: testMetadata.testFramework,
      edgeCaseCount: mergedEdgeCases.length,
      edgeCases: mergedEdgeCases,
      testCode,
      testFilePath: normalizeWorkspacePath(testMetadata.testFilePath),
