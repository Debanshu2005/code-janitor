...
try {
  ...
  try {
    parse5 = require(path.join(__dirname, '..', '..', '..', 'node_modules', 'parse5'))
  } catch (e2) {
...
try {
  ...
  console.warn('prettier not found, HTML formatting will be limited:', error.message)
  prettier = null
}
...
class HtmlFixer {
  ...
  // Show preview if changes were made
    ...
    if (result.shouldShowPreview && this.vscode && this.filePath.endsWith('.html')) {
    try {
  ...
  // --- Core Logic ---
    ...
    const finalCode = await this.fixEmbeddedContentWithParsing(repaired, changeLog)
    ...
    const changeLog = ['Applied basic HTML fixes (advanced parsing failed)']
    ...
    warning: 'HTML Syntax Error: The code could not be fully repaired. Visual preview may be broken.'
    }
    } catch (basicErr) {
    return {
    success: false,
  ...
  // Basic HTML fixes without parse5
    ...
    if (trimmed.startsWith('<') && !trimmed.startsWith('</') && !trimmed.endsWith('/>') && !this.isVoidElementTag(trimmed)) {
    indentLevel++}
    ...
    for (let i = 0; i < node.childNodes.length; i++) {
      ...
      if (child.tagName && !this.isVoidElement(child.tagName)) {
        if (child.childNodes && this.hasUnclosedText(child)) {
          fixes.addedClosures++;const lineInfo = this.getLineNumber(child)
          changeLog.push(`Fixed unclosed <${child.tagName}> element${lineInfo}`)
        }
      }

      if (child.tagName && this.isVoidElement(child.tagName)) {
        if (child.childNodes && child.childNodes.length > 0) {
          fixes.fixedVoidElements++;const lineInfo = this.getLineNumber(child)
          changeLog.push(`Removed invalid content from void <${child.tagName}> element${lineInfo}`)
          child.childNodes = []
        }
      }
      ...
      if (nestingFix) {
        fixes.fixedNesting++;const lineInfo = this.getLineNumber(child)
        changeLog.push(`Fixed invalid nesting: <${child.tagName}> inside <${node.tagName}>${lineInfo}`)
      }
    }
