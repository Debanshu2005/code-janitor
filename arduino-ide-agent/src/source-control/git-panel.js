...
class GitPanel {
  ...
  async cloneRepository() {
    ...
    try {
      ...
      this.sendMessage({
        type: "status",
        text: `✓ Repository cloned to ${repoPath}`
      })
  ...
  async stashChanges() {
    try {
      ...
      this.sendMessage({
        type: "error",
        text: `Stash failed: ${error.message}`
      })
    }
  }
  ...
  async popStash() {
    try {
      ...
      this.sendMessage({
        type: "error",
        text: `Pop stash failed: ${error.message}`
      })
    }
  }
  ...
  async viewHistory() {
    try {
      ...
      vscode.window.showInformationMessage("Recent commits:", {
        modal: true,
        detail: log
      })
    } catch (error) {
      this.sendMessage({
        type: "error",
        text: `View history failed: ${error.message}`
      })
    }
  }
  ...
  async abortMerge() {
    try {
      ...
      this.sendMessage({
        type: "error",
        text: `Abort merge failed: ${error.message}`
      })
    }
  }
  ...
  async acceptIncoming() {
    try {
      const conflicts = await this.executeGit(
      ...
      this.sendMessage({
        type: "error",
        text: `Accept incoming failed: ${error.message}`
      })
    }
  }
  ...
  async acceptCurrent() {
    try {
      ...
      this.sendMessage({
        type: "error",
        text: `Accept current failed: ${error.message}`
      })
    }
  }
  ...
  async gitPush() {
    ...
    try {
      ...
      if (!hasUpstream) {
        ...
        this.sendMessage({
          type: "status",
          text: `✓ Pushed and set upstream to origin/${currentBranch}`
        })
        ...
        try {
          ...
          vscode.window
            .showInformationMessage(`Pushed to ${remoteUrl}`, "Open on GitHub")
            .then((selection) => {
      ...
      if (
        ...
        this.sendMessage({
          type: "error",
          text: "Push failed: No upstream branch configured"
        })
        ...
        if (forcePush === "Pull First") {
          ...
          try {
            ...
            this.sendMessage({
              type: "status",
              text: "✓ Force pushed successfully"
            })
            await this.refreshStatus()
          } catch (e) {
            this.sendMessage({
              type: "error",
              text: `Force push failed: ${e.message}`
            })
          }
        }
      } else {
  ...
  async gitCommit(message) {
    ...
    this.sendMessage({
      type: "status",
      text: "Staging and committing changes..."
    })
    ...
    try {
      ...
      if (isMerging) {
        ...
        if (conflicts.trim()) {
          ...
          this.sendMessage({
            type: "error",
            text: "Resolve conflicts before committing"
          })
          vscode.window.showErrorMessage(
      ...
      if (!status.trim() && !isMerging) {
        ...
        vscode.window.showWarningMessage(
          "No changes to commit. Modify files first."
        )
        return
      }
  ...
  async unstageFile(file) {
    try {
      ...
      this.sendMessage({
        type: "error",
        text: `Unstage failed: ${error.message}`
      })
    }
  }
  ...
  async stageAll() {
    try {
      ...
      this.sendMessage({
        type: "error",
        text: `Stage all failed: ${error.message}`
      })
    }
  }
  ...
  async unstageAll() {
    try {
      ...
      this.sendMessage({
        type: "error",
        text: `Unstage all failed: ${error.message}`
      })
    }
  }
  ...
  async discardChanges(file) {
    ...
    try {
      ...
      this.sendMessage({
        type: "error",
        text: `Discard failed: ${error.message}`
      })
    }
  }
  ...
  async createBranch() {
    ...
    try {
      ...
      this.sendMessage({
        type: "status",
        text: `✓ Created and switched to ${branchName}`
      })
      await this.refreshStatus()
    } catch (error) {
      this.sendMessage({
        type: "error",
        text: `Create branch failed: ${error.message}`
      })
    }
  }
  ...
  async configureGit() {
    try {
      ...
      this.sendMessage({
        type: "error",
        text: `Git config failed: ${error.message}`
      })
    }
  }
  ...
  async _doRefresh() {
    ...
    try {
      ...
      if (!foundGit) {
        console.log(
          "[Git Panel] .git folder not found in workspace or parent directories"
        )
        this.sendMessage({
          type: "noRepo",
      ...
      const branch = await this.executeGit(
      ...
      let conflicts = []
      try {
        const conflictFiles = await this.executeGit(
          "diff --name-only --diff-filter=U",
          gitRoot
        )
              ...
              ? refs
                  ...
                  .filter(
                    (r) =>
                      r.includes("HEAD") ||
                      r.startsWith("origin/") ||
                      (!r.includes("tag:") && !r.includes("HEAD"))
                  )
                  .map((r) =>
  ...
  getHtmlContent() {
    return fs.readFileSync(path.join(__dirname, "git-panel.html"), "utf8")
  }
}
